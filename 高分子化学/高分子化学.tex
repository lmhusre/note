...
\section{缩聚与逐步聚合}
\subsection{引言}
{\bf 缩合反应} 有机官能团之间翻译除了形成著产物以外上有低分子物产生的反应，称为缩合反应。
\begin{itemize}
\item {参加缩合反应的反应物中，每个分子仅有一个参加反应的官能团}
\item {缩合反应只能形成低分子化合物}
\item 当每个参加反应的的分子中的官能团数大于等于2时，才能能形成聚合物，为缩聚反应
\item 缩聚反应是通过官能团之间的多次缩合反应形成聚合物的过程
\end{itemize}
...
逐步聚合是高分子合成最基本的类型之一，逐步聚合的基本特征是官能团之间的反应。聚合度随时间逐步增长，而转化率在集合出去便可到达很高，从而表现出与连锁聚合不同的规律
\subsubsection{逐步聚合的类型}
（1）缩聚反应：官能团之间的缩合聚合反应，同时有小分子产生

（2）聚加成反应：形式上是加成反应，机理是逐步的。
...
\subsection{缩聚反应}官能团之间进过多次缩合形成聚合物的反应
...
缩聚反应过程中往往存在联众环化反应，单体单元内环化与分子内环化。

环化反应发生在同一单元体内，分子内环化发生在同一分子内的不同单元体之间。环化反应会降低聚合度，影响产物的性能。
\subsubsection{线形缩聚机理}
缩聚反应的重要特征之一是大分子链增长是逐步进行的。比如以aAa代表二元醇，bBb他们形成聚酯的过程如下：
...
\paragraph{溶胶与凝胶}溶胶为线形或者支化的能溶型高分子，凝胶为不能溶解的交联高分子，出现凝胶点时并非所有基团都已反应，既有凝胶也有溶胶。
...
4)3-2体系A要与B反应两次才能支化，则凝结支化概率是2次反应的概率$$\alpha_c = {p_c}^2=0.5$$
\paragraph{普遍情况}
...
及端基$A_f$与B-B缩聚；端基B与A-A缩聚，反复n次，最后B与$A_f$反应形成体型产物。结果两末端为支化单元，方括号里为线形链段,反应概率为各步骤概率乘积
...
\begin{itemize}
  \item 绝大多数逐步聚合物为杂链聚合物，可分为线形与体形两类。
  \item 线形逐步聚合物一般可溶可熔，如聚酯、聚酰胺、聚碳酸酯等；体形逐步聚合物一般不溶不熔，如醇酸树脂、酚醛树脂、氨基树脂等，其合成多采用“预聚+固化成型”两阶段。
  \item 不同品种逐步聚合物在结构单元形成机理与动力学规律上基本相同，而性能差异主要取决于结构单元自身结构、数量及其分布，以及共聚组成、序列分布和分子拓扑形状（线形、星形、树形、网络形等）。
\end{itemize}

\subsubsection{聚酯（PE）}
\begin{itemize}
  \item 主链含酯基（$-\mathrm{COO}-$）的杂链聚合物；仅含酯侧基的聚合物不归入聚酯。
  \item 聚酯按结构和用途可分为：线形饱和脂族聚酯、线形芳族聚酯、不饱和聚酯和醇酸树脂。
  \item 线形饱和脂族聚酯的代表有聚酯二醇和聚乳酸；线形芳族聚酯包括半芳族的涤纶聚酯、工程塑料聚酯以及全芳族的聚对羟基苯甲酸等。
  \item 不饱和聚酯主链中保留$\mathrm{C=C}$双键，通常作为结构预聚物溶于苯乙烯后再经自由基聚合交联成网状高分子，多用于增强塑料；醇酸树脂则为线形或支链形无规预聚物，残留基团可继续交联固化，主要用作涂料。
  \item 涤纶聚酯PET目前主要采用直接酯化法：精对苯二甲酸（PTA）与乙二醇（EG）在路易斯酸催化下，先于约$200\,^{\circ}\mathrm{C}$、常压或负压下酯化生成乙二醇封端的对苯二甲酸乙二醇酯，再于约$270\,^{\circ}\mathrm{C}$、负压下缩聚，最后在更高温度和更高真空度下终缩聚，得到高聚合度PET。
  \item PET是合成纤维第一大品种，具有熔点高、强度好、耐溶剂、耐腐蚀和手感好等特点；同类产品还有PBT、PTT等。
\end{itemize}
...
\begin{itemize}
  ...
  \item 尼龙-66由己二胺与己二酸缩聚而成。由于氨基比羟基活泼，缩聚时一般无需催化剂；又因平衡常数较大，反应几乎可视为不可逆，因此工业上可采用“水中预缩聚+熔融终缩聚”的两段法。预缩聚时先在水中中和制得66盐，以保证等基团数比、纯化体系并防止胺挥发，再在约$200\,^{\circ}\mathrm{C}$、$1.5\,\mathrm{MPa}$下预缩聚到反应程度约$0.8\sim 0.9$；随后闪蒸除水，在高于熔点的约$275\,^{\circ}\mathrm{C}$和负压下终缩聚。
  \item 尼龙-66熔点高（约$265\,^{\circ}\mathrm{C}$），机械强度高，兼具刚性与柔韧性，摩擦系数和蠕变性低，且耐磨、耐溶剂、易染色；既可作为工程塑料，也可纺丝制成锦纶。
  \item 芳族聚酰胺因主链引入苯环，耐热性和刚性较脂族体系明显提高。其中对位连接可形成刚性伸直链和较强共轭作用，表现为高强度、高模量；间位连接则链段柔性相对较好。典型代表包括半芳的尼龙-6T和全芳的对位芳纶PPTA。
  \item 2系列脂族聚酰胺以尼龙-6为代表。由于己内酰胺开环聚合速率远高于氨基酸自缩聚，工业上几乎均采用己内酰胺开环聚合来制备尼龙-6；该反应可采用碱催化或水（酸）催化。碱催化开环聚合属于负离子聚合，所得产品机械强度更高，韧性好，低蠕变、耐磨耗、化学稳定性好，可直接用于浇铸尼龙制品。
\end{itemize}
...
\begin{itemize}
  ...
  \item 聚酰亚胺一般由芳二酐与脂二胺或芳二胺缩聚而成，通常采用“两步法”：先在DMF、DMSO等强极性溶剂中进行预缩聚，得到可加工的液态预聚物；再在成膜、成纤等成型后加热终缩聚并成环固化。
  \item 当链段中的$-R-$为脂族结构时，最终产物往往可溶可熔；若$-R-$为芳环结构，则最终产物通常不溶不熔。
  \item 这类高性能逐步聚合物还包括聚苯并咪唑（PBI）和梯形聚合物。PBI通常由芳族四元胺与二元芳酸或其酯缩聚而成，也常采用“预缩聚+终缩聚”的两步法；梯形聚合物则因主链结构更接近双链或梯形，耐热性更高。
\end{itemize}

\subsubsection{聚氨酯与聚脲}
\begin{itemize}
  \item 聚氨酯是典型的含氮杂链聚合物，全名为聚氨基甲酸酯，可以是线形也可以是体形，属于产量很大的逐步聚合物。
  \item 理论上聚氨酯有两条合成路线：一是二氯代甲酸酯与二元胺的低温界面缩聚；二是二异氰酸酯与二元醇的聚加成。由于后一方法无副产物、二元醇更易处理，工业上主要采用二异氰酸酯与二元醇聚加成路线。
  \item 线形聚氨酯的代表是热塑性聚氨酯TPU和氨纶纤维。其分子链通常呈软段-硬段嵌段结构，硬段含芳环并可结晶、形成强氢键，相当于可逆物理交联点；软段提供柔韧性，因此材料既有弹性，又能熔融加工。
  \item 体形聚氨酯包括泡沫塑料、涂料和聚氨酯橡胶等，一般采用“预聚+交联”的两步工艺。预聚阶段多由二异氰酸酯与聚醚二醇或聚酯二醇反应，生成异氰酸酯封端预聚物；交联阶段可通过多羟基化合物、胺类扩链剂等进一步交联。
  \item 聚氨酯泡沫塑料可分为软泡和硬泡。软泡为开孔结构，常用于沙发、枕头和隔音内衬等；硬泡为闭孔结构，常用于冷库、冰箱、管道等保温材料。聚氨酯涂料又可分为单组分和双组分体系。
  \item 聚氨酯橡胶以线形异氰酸酯封端预聚物为基础，经化学交联形成弹性体；与TPU相比，其力学性能、耐热性和耐溶剂性更好，但属于不可逆交联体系，难以重复加工。
...
\begin{itemize}
  ...
  \item 聚苯醚（PPO）是线形全芳醚聚合物，属于性能优异的工程塑料，常由2,6-二甲基苯酚经氧化耦合反应制得。
\end{itemize}

\subsubsection{聚砜与其他含硫杂链聚合物}
\begin{itemize}
  \item 聚砜（PSF）是主链中含砜基（$-\mathrm{SO_2}-$）的线形非晶态聚合物，分为脂族和芳族两类。脂族聚砜$T_g$较低、热稳定性差，应用有限；芳族聚砜常称聚芳醚砜或聚芳砜，具有较高$T_g$、较好的刚性和强度，属于工程塑料。
...
\begin{itemize}
  ...
  \item 酸催化酚醛预聚物称Novolacs，通常在苯酚过量条件下制备。其机理是甲醛羰基先质子化，再在苯酚邻、对位发生亲电芳香取代，继而缩合形成由亚甲基桥随机连接的热塑性酚醛树脂。该类预聚物本身不会直接交联，需与乌洛托品混合后加热，借由其分解提供亚甲基而发生交联固化。
\end{itemize}
...
\begin{enumerate}[1)]
  ...
  \]
\end{enumerate}
特点：分解常为吸热过程，$E_d$较高，$k_d$较小，引发通常是速率控制步骤。
...
\]
链转移使原活性链失活，新的自由基可能继续引发；会降低分子量，并可能引入支化（如LDPE中向大分子转移）。
...
\begin{itemize}
  \item 偶氮类：如AIBN，分解放出$\mathrm{N_2}$，便于测定分解速率，活性常偏低至中等。
  \item 有机过氧类：如BPO、DCP、过氧酯、过氧碳酸酯，适用温区广。
  \item 无机过氧类：如过硫酸盐，水溶性好，适于乳液/水溶液聚合。
\end{itemize}
...
\subsubsection{辐射、等离子体与微波引发}
\begin{itemize}
  \item 辐射引发：能量高、穿透强，可用于固相聚合。
  \item 等离子体引发：可用于表面改性、成膜、固定化等。
  \item 微波引发：兼具热效应和非热效应，可加速聚合并降低引发剂用量。
\end{itemize}
...
\begin{itemize}
  \item NMP：氮氧稳定自由基调聚法，机理直观，但单体适用范围较窄，常需较高温。
  \item ATRP：原子转移可逆失活，单体适用广，需金属络合催化体系。
  \item RAFT：可逆加成-裂解-链转移法，条件接近传统自由基聚合，适用单体范围广。
\end{itemize}
...
\subsubsection{概述}
工业自由基聚合主要包括：本体聚合、溶液聚合、沉淀/分散聚合、悬浮聚合和乳液聚合。

\subsubsection{本体聚合}
\begin{itemize}
  \item 优点：产品纯净、设备相对简单、可连续化。
  \item 缺点：体系黏度高、传热困难、易自动加速与爆聚。
  \item 典型产品：GPPS、SAN、R-SMA、HIPS、ABS、PMMA、LDPE。
\end{itemize}
...
\begin{itemize}
  ...
  \item 典型应用：腈纶纺丝液、PVAc/EVA体系、涂料树脂溶液。
\end{itemize}

\subsubsection{沉淀或分散聚合}
引发剂与单体在介质中互溶，但聚合物不溶，产物易分离纯化。典型体系：丙烯腈水相沉淀聚合、TFE分散聚合（PTFE）。

\subsubsection{悬浮聚合}
\begin{itemize}
  \item 组分：单体、油溶性引发剂、水、分散剂。
  \item 反应场所在单体液滴中，动力学近似本体聚合。
  \item 典型产品：PVC悬浮树脂、EPS珠粒等。
\end{itemize}

\subsubsection{乳液聚合}
\begin{itemize}
  \item 组分：油溶性单体、水溶性引发剂、水、乳化剂。
  \item 反应场所：增溶胶束或乳胶粒。
  \item 特点：传热好、速率快、可得高分子量，部分乳液可直接应用。
\end{itemize}
经典乳液聚合分三阶段：
\begin{enumerate}[1)]
...
\begin{itemize}
  \item 传统应用：合成橡胶胶乳、ABS/MBS改性体系、涂料和胶黏剂胶乳。
  \item 新技术：种子乳液、核壳乳液、活性乳液、细乳液、微乳液、反相乳液、无皂乳液等。
\end{itemize}
