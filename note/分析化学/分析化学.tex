...
\subsection{酸碱质子理论与共轭关系}

\subsubsection{质子理论的核心表述}
在酸碱滴定中采用布朗斯特-劳里质子理论：
\begin{itemize}
    \item \textbf{酸}：能给出质子的物质；
    \item \textbf{碱}：能接受质子的物质；
    \item \textbf{两性物质}：既可给出质子，也可接受质子。
\end{itemize}
...
\subsubsection{共轭酸碱对与酸碱反应本质}
共轭酸碱对由“得失一个质子”相互转化而成，如 $\mathrm{HA/A^-}$、$\mathrm{NH_4^+/NH_3}$。

完整酸碱反应由两个半反应组成，其本质是质子传递：
\begin{equation}
\mathrm{HA}+\mathrm{B}\rightleftharpoons \mathrm{A^-}+\mathrm{BH^+}
\end{equation}

\subsubsection{水的自递质子与离子积}
\begin{equation}
...
\mathrm{p}K_w=-\lg K_w
\end{equation}
温度升高时，$K_w$ 增大、$\mathrm{p}K_w$ 减小，因此“中性溶液 pH = 7.00”只严格成立于
$25\,^{\circ}\mathrm{C}$。

\subsubsection{弱酸弱碱的解离常数与共轭关系}
对一元弱酸、弱碱分别有：
\begin{equation}
...
据此可得：
\begin{itemize}
    \item $K_a$ 越大，酸性越强；$K_b$ 越大，碱性越强；
    \item 弱酸越强，其共轭碱越弱；弱碱越强，其共轭酸越弱；
    \item 比较酸碱强弱时，应在同一溶剂体系中进行。
\end{itemize}

\subsection{分布分数与优势形态判断}

\subsubsection{1. 一元弱酸分布分数}
对 $\mathrm{HA}\rightleftharpoons\mathrm{H^+}+\mathrm{A^-}$：
\begin{equation}
\delta_{\mathrm{HA}}=\frac{[\mathrm{HA}]}{c_{\mathrm{HA}}}=\frac{[\mathrm{H^+}]}{[\mathrm{H^+}]+K_a}
\end{equation}
\begin{equation}
\delta_{\mathrm{A^-}}=\frac{[\mathrm{A^-}]}{c_{\mathrm{HA}}}=\frac{K_a}{[\mathrm{H^+}]+K_a}
\end{equation}
\begin{equation}
\delta_{\mathrm{HA}}+\delta_{\mathrm{A^-}}=1
\end{equation}

要点：
\begin{itemize}
    \item 对给定弱酸，分布分数只与 $\mathrm{pH}$ 和 $K_a$ 有关；
    \item $\mathrm{pH}=\mathrm{p}K_a$ 时，$\delta_{\mathrm{HA}}=\delta_{\mathrm{A^-}}=0.5$；
    \item $\mathrm{pH}<\mathrm{p}K_a$ 时酸式占优，$\mathrm{pH}>\mathrm{p}K_a$ 时碱式占优。
\end{itemize}

\subsubsection{2. 多元弱酸分布分数的用途}
多元弱酸有多个型体（如 $\mathrm{H_2A}$、$\mathrm{HA^-}$、$\mathrm{A^{2-}}$），通过 $\delta$ 可判断不同 pH 下主导型体。
...
\subsubsection{3. 分布曲线与优势区域}
以 $\delta$ 为纵坐标、pH 为横坐标可得分布曲线。它有两个直接用途：
\begin{itemize}
    \item 快速判断某 pH 下的优势型体；
    \item 直观看出不同型体在何处交替占优。
\end{itemize}

对一元弱酸，$\mathrm{pH}=\mathrm{p}K_a$ 时两条分布曲线相交；对多元酸，相邻型体的交点通常对应相邻的
$\mathrm{p}K_a$，因此常可借此判断缓冲区间与分步滴定可行性。

\subsection{酸碱溶液 pH 计算的通用框架}

\subsubsection{1. 三大平衡方程}
\begin{itemize}
    \item \textbf{物料平衡（MBE）}：某组分分析浓度等于各存在型体平衡浓度之和；
    \item \textbf{电荷平衡（CBE）}：溶液正负电荷总量相等；
    \item \textbf{质子条件式（PBE）}：得失质子总量守恒。
\end{itemize}

实际计算通常是：先写 PBE，再联立解离平衡式和必要的物料平衡式，最后作合理近似。

\subsubsection{2. 强酸（强碱）溶液的 pH}
以一元强酸浓度 $c$ 为例：
\begin{itemize}
    \item 当 $c>10^{-6}\,\mathrm{mol\cdot L^{-1}}$，可近似 $[\mathrm{H^+}]\approx c$；
    \item 当 $c<10^{-8}\,\mathrm{mol\cdot L^{-1}}$，水解离主导，$[\mathrm{H^+}]\approx 10^{-7}$；
    \item 中间区间需用精确式求解。
\end{itemize}

\subsubsection{3. 一元弱酸（弱碱）溶液的 pH}
对一元弱酸（分析浓度 $c_a$）：
\begin{equation}
[\mathrm{H^+}]^2+K_a[\mathrm{H^+}]-K_ac_a=0
\end{equation}
若满足 $c_aK_a\gg K_w$ 且 $c_a/K_a\gg 1$，则常用最简式：
\begin{equation}
[\mathrm{H^+}]\approx \sqrt{K_ac_a}
\end{equation}

弱碱可类比处理，先求 $[\mathrm{OH^-}]$ 再由
\begin{equation}
\mathrm{pH}=14.00-\mathrm{pOH}
\end{equation}
得到 pH。

\subsubsection{4. 多元酸、酸式盐与弱酸弱碱盐的简化判据}
\begin{itemize}
    \item 多元酸常以一级解离主导近似；
    \item 酸式盐（两性物质）在满足数量级条件时常用近似：
\begin{equation}
[\mathrm{H^+}]\approx \sqrt{K_{a1}K_{a2}}
\end{equation}
    \item 弱酸弱碱盐（如 $\mathrm{NH_4Ac}$）在盐浓度足够且可忽略水自解离时，可近似写为：
\begin{equation}
[\mathrm{H^+}]\approx \sqrt{K_a(\text{阳离子酸})K_a(\text{阴离子共轭酸})}
\end{equation}
    \item 若阳离子酸更强，则溶液偏酸；若阴离子共轭酸更强，则溶液偏碱。
\end{itemize}

\subsection{缓冲溶液}

\subsubsection{1. 含义与组成}
缓冲溶液可在少量加酸、加碱或轻度稀释时，保持 pH 基本不变。

常见组成：
\begin{itemize}
    \item 弱酸-共轭碱（如 $\mathrm{HAc/Ac^-}$）；
    \item 弱碱-共轭酸（如 $\mathrm{NH_3/NH_4^+}$）。
\end{itemize}

\subsubsection{2. 缓冲机理与 Henderson-Hasselbalch 方程}
以 $\mathrm{HAc/Ac^-}$ 为例：
\begin{equation}
...
通式：
\begin{equation}
\mathrm{pH}=\mathrm{p}K_a+\lg\frac{[\text{base}]}{[\text{acid}]}
\end{equation}

机理速记：酸来碱挡、碱来酸挡。

\subsubsection{3. 缓冲容量与使用原则}
缓冲容量定义：
\begin{equation}
\beta=\frac{\mathrm{d}b}{\mathrm{d}pH}=-\frac{\mathrm{d}a}{\mathrm{d}pH}
\end{equation}
其中 $\mathrm{d}b$、$\mathrm{d}a$ 分别是每升缓冲液引起 pH 变化单位量所需加入强碱或强酸的物质的量。

原则：
\begin{enumerate}
    \item 工作 pH 落在有效范围 $\mathrm{p}K_a\pm 1$；
    \item 组分总浓度足够（浓度越大缓冲能力越强）；
    \item 酸碱组分比接近 $1:1$ 时缓冲容量最大；
    \item 缓冲组分不与体系中关键离子发生副反应。
\end{enumerate}

\subsubsection{4. 缓冲液配制流程}
\begin{enumerate}
    \item 按目标 pH 与总浓度，用 Henderson 方程计算酸碱配比；
    \item 先按计算量配制；
    \item 用 pH 计实测后微调（少量酸/碱修正）。
\end{enumerate}

\subsubsection{5. 常见缓冲体系与应用提示}
\begin{itemize}
    \item 乙酸-乙酸根缓冲液：适用于弱酸性区；
    \item 磷酸二氢根-磷酸氢根缓冲液：适用于近中性区；
    \item 氨-铵盐缓冲液：适用于碱性区，如配位滴定中常见的 pH 10 条件。
\end{itemize}

课件中常见的酸度计标准缓冲液（$25\,^{\circ}\mathrm{C}$）可记为：
\begin{itemize}
    \item 邻苯二甲酸氢钾：$\mathrm{pH}=4.00$；
    \item 磷酸盐标准缓冲液：$\mathrm{pH}=6.86$；
    \item 硼砂标准缓冲液：$\mathrm{pH}=9.18$。
\end{itemize}

此外，生化实验中常见 PBS（磷酸盐缓冲盐溶液），其意义更多在于提供稳定近中性的离子环境，而不仅是单纯“算 pH”。

\subsection{酸碱指示剂}

\subsubsection{1. 作用原理}
酸碱指示剂本质上多为有机弱酸或有机弱碱，可表示为：
\begin{equation}
\mathrm{HIn}\rightleftharpoons \mathrm{H^+}+\mathrm{In^-}
\end{equation}
酸式与碱式颜色不同，当二者比例变化到可辨识阈值时出现变色。
...
\subsubsection{2. 常用指示剂（课件重点）}
\begin{itemize}
    \item 甲基橙：$3.1\sim 4.4$（红$\rightarrow$黄）；
    \item 甲基红：$4.4\sim 6.2$（红$\rightarrow$黄）；
    \item 溴麝香草酚蓝：$6.0\sim 7.6$（黄$\rightarrow$蓝）；
    \item 酚红：$6.7\sim 8.4$（黄$\rightarrow$红）；
    \item 酚酞：$8.0\sim 9.6$（无色$\rightarrow$红）。
\end{itemize}

\subsubsection{3. 实际变色范围与混合指示剂}
实际变色范围通常略窄于理论值，很多常用指示剂的实际变色区间约为
$1.6\sim 1.8$ 个 pH 单位。

\begin{itemize}
    \item 双色指示剂：变色范围基本不受指示剂用量影响；
    \item 单色指示剂：浓度增大时，往往在较低 pH 就出现可见颜色；
    \item 混合指示剂：可利用颜色互补使终点颜色变化更敏锐，缩窄可观察区间。
\end{itemize}

\subsubsection{4. 使用注意事项}
\begin{enumerate}
    \item 按滴定突跃区间选指示剂，而不是只看名字；
    \item 指示剂用量宜少不宜多（避免改变体系 pH 与视觉迟钝）；
    \item 变色范围受温度影响，通常在室温下使用；
    \item 需要更窄变色区间时可用混合指示剂。
\end{enumerate}

\subsection{酸碱滴定曲线与突跃}

\subsubsection{1. 滴定分数与曲线表达}
定义滴定分数：
\begin{equation}
a=\frac{n_t}{n_d}=\frac{c_tV_t}{c_dV_d}
\end{equation}
以 $a$ 为横坐标、pH 为纵坐标作图可得滴定曲线。

\subsubsection{2. 强碱滴定强酸（核心结论）}
以 $0.1000\,\mathrm{mol\cdot L^{-1}}\ \mathrm{NaOH}$ 滴定
$20.00\,\mathrm{mL}$ 同浓度 $\mathrm{HCl}$ 为例：
\begin{itemize}
    \item 滴定前 $a=0$，$\mathrm{pH}=1.00$；
    \item 计量点前 $a=0.999$，$\mathrm{pH}\approx 4.30$；
    \item 计量点 $a=1.000$，$\mathrm{pH}=7.00$；
    \item 计量点后 $a=1.001$，$\mathrm{pH}\approx 9.70$。
\end{itemize}
可见在很窄体积区间内 pH 急变，形成滴定突跃。

\subsubsection{3. 强碱滴定一元弱酸（以 HAc 为例）}
曲线特点：
\begin{itemize}
    \item 起始 pH 高于强酸体系；
    \item 计量点前形成缓冲区（曲线较平缓）；
    \item 半中和点满足 $\mathrm{pH}=\mathrm{p}K_a$；
    \item 计量点 pH 大于 7（由共轭碱水解决定）；
    \item 突跃较强酸体系更小。
\end{itemize}
...
\subsubsection{4. 多元弱酸分步滴定判据}
对多元弱酸，常用两个判据：
\begin{equation}
cK_{a1}\ge 10^{-8}
\end{equation}
\begin{equation}
\frac{K_{a1}}{K_{a2}}>10^5
\end{equation}
前者保证一级可滴定，后者保证相邻两步解离可分辨。

\subsubsection{5. 浓度对滴定突跃的影响}
\begin{itemize}
    \item 在同类滴定体系中，酸碱浓度越高，滴定突跃通常越大；
    \item 体系越稀，计量点附近 pH 变化越平缓，指示剂选择余地越小；
    \item 因此低浓度滴定时更需要关注终点误差，必要时改用更合适的指示剂或仪器法判定终点。
\end{itemize}

\subsection{终点误差与指示剂选择}

\subsubsection{1. 终点误差定义}
滴定终点（ep）与化学计量点（sp）不重合引起的误差：
\begin{equation}
E_t=\frac{n_{t,ep}-n_{d,ep}}{n_{d,ep}}\times 100\%
\end{equation}

\subsubsection{2. 强碱滴定强酸的林邦公式}
\begin{equation}
E_t=\frac{10^{\Delta \mathrm{pH}}-10^{-\Delta \mathrm{pH}}}{\dfrac{K_w}{c_{\mathrm{HCl}}^{sp}}}\times 100\%
\end{equation}
其中
\begin{equation}
\Delta \mathrm{pH}=\mathrm{pH}_{ep}-\mathrm{pH}_{sp}
\end{equation}
公式说明：终点越偏离计量点，误差越大；计量点处浓度越低，误差放大越明显。

\subsubsection{3. 强碱滴定弱酸的误差表达式}
\begin{equation}
E_t=\frac{10^{\Delta \mathrm{pH}}-10^{-\Delta \mathrm{pH}}}{\left(\dfrac{K_a}{K_w}\right)c_{\mathrm{HA}}^{sp}}
\end{equation}
该式体现了弱酸强度（$K_a$）和计量点浓度对终点误差的共同影响。

\subsubsection{4. 指示剂选择总原则}
\begin{enumerate}
    \item 指示剂实际变色范围应落在滴定突跃区；
    \item 优先选择使 $|\Delta \mathrm{pH}|$ 更小的指示剂；
    \item 平行实验中指示剂加入量保持一致。
\end{enumerate}

\summarytitle
\begin{enumerate}
    \item 酸碱滴定以质子传递为核心，掌握共轭酸碱对是理解反应与计算的基础。
    \item $K_a/K_b$ 与 $K_w$ 的关系可用于串联理解酸碱强弱、共轭关系与后续近似公式。
    \item 分布分数与分布曲线可快速判断型体分布，是缓冲、滴定和条件选择的核心工具。
    \item pH 计算遵循 PBE/MBE/CBE 框架，并根据数量级条件选精确式、近似式或最简式。
    \item 缓冲溶液的计算核心是 Henderson 方程，使用核心是 $\mathrm{p}K_a\pm 1$、缓冲容量与体系兼容性。
    \item 指示剂选择本质是“变色范围匹配突跃范围”，并兼顾温度、用量和混合指示剂的适用性。
    \item 强酸强碱、强碱弱酸、多元酸体系的曲线特征、浓度效应和可滴定判据必须区分掌握。
    \item 终点误差与 $\Delta\mathrm{pH}$、计量点浓度密切相关，公式计算与图示判断应结合使用。
\end{enumerate}

\subsection{例题模板区（第四章）}

\begin{exampletemplate}{模板 D：缓冲液 pH 与配比计算}
    \item 明确缓冲对（弱酸-共轭碱或弱碱-共轭酸），写 Henderson 方程。
    \item 根据目标 pH 求浓度比：
\begin{equation}
    \frac{[\text{base}]}{[\text{acid}]}=10^{\mathrm{pH}-\mathrm{p}K_a}
\end{equation}
    \item 联立总浓度条件求各组分浓度，再换算配制用量。
    \item 配后用 pH 计实测，记录微调体积与最终 pH。
\end{exampletemplate}

\begin{exampletemplate}{模板 E：滴定曲线关键点快速计算}
    \item 计算滴定分数 $a=\dfrac{c_tV_t}{c_dV_d}$，判断处于滴定前、计量点前、计量点或计量点后。
    \item 计量点前后按“剩余组分控制 pH”处理；弱酸体系在缓冲区可用 Henderson 方程。
    \item 计量点处先判定主导平衡（中性盐、弱碱盐或弱酸盐），再求 pH。
    \item 至少给出 $a=0.999,1.000,1.001$ 三点，用于估算突跃范围与指示剂适配性。
\end{exampletemplate}

\begin{exampletemplate}{模板 F：终点误差评估与指示剂优选}
    \item 根据体系类型选误差公式（强酸强碱或强碱弱酸）。
    \item 由指示剂变色点估算 $\mathrm{pH}_{ep}$，计算
\begin{equation}
    \Delta\mathrm{pH}=\mathrm{pH}_{ep}-\mathrm{pH}_{sp}
\end{equation}
    \item 代入公式计算 $E_t$，比较不同指示剂对应误差大小。
    \item 选取 $|E_t|$ 更小且颜色变化清晰的方案，并注明判据。
\end{exampletemplate}
