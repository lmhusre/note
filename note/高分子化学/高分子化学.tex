...
\end{itemize}
无链转移时，终止方式还决定链端结构与聚合度表达：
\begin{itemize}
  \item 偶合终止：所得大分子由两条活性链结合而成，$DP$为两条链重复单元数之和，大分子两端通常都带引发剂残基。
  \item 歧化终止：每条大分子只对应一条活性链，$DP$与该链重复单元数相同；产物一端为引发剂残基，另一端表现为饱和端或不饱和端。
\end{itemize}
...
\begin{itemize}
  ...
  \item 少量阻聚剂（约$0.01\%\sim0.1\%$）即可显著抑制甚至终止自由基聚合。
\end{itemize}
...
\begin{table}[!t]
  ...
  \begin{tabularx}{\textwidth}{C{2.2cm}C{3.0cm}C{2.0cm}Y}
    \toprule
    \rowcolor{blue!6}
    \textbf{类别} & \textbf{代表引发剂} & \textbf{活性} & \textbf{复习关键词} \\
    \midrule
    偶氮类 & AIBN，ABVN & 低到中 & 一级分解，放出$\mathrm{N_2}$，便于测定分解速率；多用于本体和溶液聚合。 \\
    有机过氧二酰/二烷基 & BPO，DCP & 低 & 油溶性常见；BPO是过氧类高频代表。 \\
    过氧酯/二碳酸酯 & BPP，DCPD & 中到高 & 半衰期更短，适于较低温或需要较快引发的体系。 \\
    无机过氧类 & 过硫酸钾，过硫酸铵 & 水溶性 & 常用于乳液、水溶液聚合，也常与还原剂组成氧化-还原体系。 \\
    \bottomrule
  \end{tabularx}
  }
\end{table}
常见课堂高频引发剂的实务特征可进一步记为：
\begin{table}[!htbp]
  \centering
  \caption{第三章高频引发剂实务对照}
  {\small
  \setlength{\tabcolsep}{5pt}
  \renewcommand{\arraystretch}{1.2}
  \begin{tabularx}{\textwidth}{C{2.2cm}C{1.9cm}C{2.2cm}Y}
    \toprule
    \rowcolor{blue!6}
    \textbf{引发剂} & \textbf{典型活性} & \textbf{溶解性/体系} & \textbf{复习要点} \\
    \midrule
    AIBN & 低 & 油溶性，本体/溶液常见 & 一级分解，放$\mathrm{N_2}$，诱导分解不明显。 \\
    BPO & 低 & 油溶性，本体/悬浮常见 & 过氧类经典代表，常用于苯乙烯类体系。 \\
    过硫酸盐（KSP/ASP） & 中到高（温度相关） & 水溶性，乳液/水溶液常见 & 可单独使用，也常与还原剂组成氧化-还原体系。 \\
    氧化-还原体系 & 低温高效 & 水相/油相均可设计 & 自由基来自电子转移，活化能低，适合低温快速引发。 \\
    \bottomrule
  \end{tabularx}
  }
\end{table}
...
\end{itemize}
\begin{itemize}
  \item 过氧化氢/亚铁盐体系中，$1$分子氧化剂通常只生成$1$个自由基；若还原剂过量，还可能继续与自由基反应使其失活。
  \item 过硫酸盐/亚硫酸盐或硫代硫酸盐体系常可形成两个自由基，因此尤其适合低温快速引发。
\end{itemize}
...
\begin{itemize}
  \item 过氧类引发剂常用碘量法测浓度；偶氮类常通过测定分解放出的$\mathrm{N_2}$体积求分解量。
  \item $A_d$通常在$10^{13}\sim10^{14}\,\mathrm{s^{-1}}$量级，$E_d$常为$105\sim150\,\mathrm{kJ\cdot mol^{-1}}$。
  \item 实验上可作$\ln([I]/[I]_0)$-$t$图求$k_d$，作$\ln k_d$-$1/T$图求$E_d$。
\end{itemize}
...
\begin{itemize}
  \item 若体系纯度较高且几乎无阻聚杂质，诱导期可明显缩短，甚至不明显。
\end{itemize}
...
\subsubsection{不同引发机理下速率方程（复习补充）}
在稳态假设与长链假设下，自由基聚合速率的一般形式可写为：
\[
R_p=k_p[M][M\cdot],\qquad R_i=R_t=2k_t[M\cdot]^2
\]
不同引发路径的核心差异体现在$R_i$对体系变量的依赖。常见幂次关系可归纳为：
\begin{table}[!htbp]
  \centering
  \caption{常见引发机理下的速率表达（幂次速记）}
  {\small
  \setlength{\tabcolsep}{6pt}
  \renewcommand{\arraystretch}{1.2}
  \begin{tabularx}{\textwidth}{C{3.3cm}Y Y}
    \toprule
    \rowcolor{blue!6}
    \textbf{情形} & \textbf{典型引发速率形式} & \textbf{聚合速率幂次关系} \\
    \midrule
    常规引发剂分解引发 & $R_i\propto [I]$ & $R_p\propto [I]^{1/2}[M]$ \\
    单体自由基形成为慢步骤 & $R_i\propto [I][M]$ & $R_p\propto [I]^{1/2}[M]^{3/2}$ \\
    光/辐射引发（记有效引发强度为$\Psi$） & $R_i\propto \Psi$ & $R_p\propto \Psi^{1/2}[M]$ \\
    \bottomrule
  \end{tabularx}
  }
\end{table}
...
\]
通常$E>0$，故温度升高，$R_p$增大；若为热引发体系，总活化能也常在$80\sim96\,\mathrm{kJ\cdot mol^{-1}}$范围内，因此升温同样会明显加快速率。但聚合度一般随温度升高而下降（见后文）。
...
诱导期结束后，若阻聚剂已基本耗尽，聚合速率通常恢复到原有水平；而缓聚剂会在更长时间内持续降低聚合速率。
\begin{itemize}
  \item 有些化合物兼具阻聚和缓聚作用，先表现为诱导期，后续仍使聚合反应变慢。
  \item 单体在储存和运输时常预先加入阻聚剂防止自聚；正式聚合前通常要用减压蒸馏或萃取等方法除去。
\end{itemize}
...
\begin{itemize}
  ...
  \item 氧在低温（常以$100^\circ\mathrm{C}$以下讨论）多表现为阻聚剂，而高温下也可能参与引发过程。
\end{itemize}
\begin{table}[!htbp]
  \centering
  \caption{阻聚剂类型与作用机理对照}
  {\small
  \setlength{\tabcolsep}{6pt}
  \renewcommand{\arraystretch}{1.2}
  \begin{tabularx}{\textwidth}{C{2.4cm}Y Y}
    \toprule
    \rowcolor{blue!6}
    \textbf{类型} & \textbf{主要机理} & \textbf{代表与复习提示} \\
    \midrule
    稳定自由基型 & 与链自由基偶联，生成低活性或非活性物种 & DPPH、TEMPO类；常用于引发速率测定。 \\
    变价金属盐型 & 通过电子转移使自由基失活 & $\mathrm{FeCl_3}$等；常见于分析与阻聚实验。 \\
    加成型 & 直接与双键自由基加成，终止增长活性 & 氧、醌类、硝基化合物等；氧的温度效应要特别记。 \\
    链转移型 & 夺氢形成稳定自由基，降低链增长概率 & 酚类、芳胺类；常兼具缓聚行为。 \\
    \bottomrule
  \end{tabularx}
  }
\end{table}
...
\begin{table}[!htbp]
  \centering
  \caption{NMP、ATRP、RAFT三类可控自由基聚合对照}
  {\small
  \setlength{\tabcolsep}{5pt}
  \renewcommand{\arraystretch}{1.2}
  \begin{tabularx}{\textwidth}{C{1.5cm}C{2.5cm}C{2.6cm}Y}
    \toprule
    \rowcolor{blue!6}
    \textbf{方法} & \textbf{核心控制步骤} & \textbf{优点} & \textbf{局限与注意} \\
    \midrule
    NMP & 烷氧胺可逆解离/复合 & 机理直观、体系较简洁 & 常需较高温，单体适用范围较窄。 \\
    ATRP & 卤原子转移可逆激活/失活 & 适用单体广，结构设计能力强 & 金属配合物残留与后处理是重点。 \\
    RAFT & 可逆加成-裂解链转移 & 条件接近常规自由基聚合，兼容性强 & 试剂颜色/气味明显，部分体系有缓聚。 \\
    \bottomrule
  \end{tabularx}
  }
\end{table}
...
\begin{table}[!t]
  ...
  \setlength{\tabcolsep}{5pt}
  \renewcommand{\arraystretch}{1.22}
  \begin{tabularx}{\textwidth}{C{1.8cm}Y Y Y}
    \toprule
    \rowcolor{blue!6}
    \textbf{方法} & \textbf{反应场所/主要组分} & \textbf{核心优缺点} & \textbf{典型产品或用途} \\
    \midrule
    本体聚合 & 单体本体中进行；多配油溶性引发剂。 & 产品纯、设备相对简单，但传热差、易凝胶效应和爆聚。 & GPPS、PMMA、LDPE等。 \\
    溶液聚合 & 单体、引发剂、聚合物与溶剂同相。 & 易控温撤热，但速率较慢，且易向溶剂链转移、溶剂回收耗能。 & 腈纶纺丝液、PVAc/EVA溶液、涂料树脂。 \\
    沉淀/分散聚合 & 单体和引发剂与介质互溶，但聚合物不溶。 & 颗粒易分离纯化，但机理比均相更复杂。 & PAN淤浆、PTFE分散体系。 \\
    悬浮聚合 & 单体液滴分散在水中；油溶性引发剂、水、分散剂。 & 动力学近似本体，传热优于本体，颗粒易分离；连续化较难。 & PVC悬浮树脂、PS珠粒、EPS。 \\
    乳液聚合 & 胶束或乳胶粒中聚合；水、水溶性引发剂、乳化剂。 & 速率快、分子量高、控温好，但后处理和脱乳化较麻烦。 & SBR、PVAc/丙烯酸酯胶乳、PVC糊树脂。 \\
    \bottomrule
  \end{tabularx}
  }
\end{table}
...
\begin{table}[!htbp]
  ...
  \setlength{\tabcolsep}{5pt}
  \renewcommand{\arraystretch}{1.22}
  \begin{tabularx}{\textwidth}{C{1.7cm}C{2.0cm}C{1.7cm}Y C{1.8cm}}
    \toprule
    \rowcolor{blue!6}
    \textbf{阶段} & \textbf{乳胶粒数} & \textbf{胶束} & \textbf{单体液滴} & \textbf{$R_p$变化} \\
    \midrule
    成核期 & 增加 & 减少并最终消失 & 数量近恒，体积逐步缩小 & 上升 \\
    恒速期 & 近恒 & 无 & 继续减少至消失 & 近恒 \\
    降速期 & 近恒 & 无 & 无 & 下降 \\
    \bottomrule
  \end{tabularx}
...
课件中的经典关系还常写为：
\[
N\propto \left(\frac{\rho}{\mu}\right)^{2/5}(a_sS)^{3/5}
\]
说明乳胶粒数既受自由基生成速率影响，也受乳化剂可覆盖表面积影响。对$\bar{n}$还应注意两类偏离经典$0$-$1$态模型的情况：
\begin{itemize}
  \item $\bar{n}<0.5$：单体水溶性较大且易链转移时，短链自由基更易解吸到水相终止；
  \item $\bar{n}>0.5$：胶粒逐渐变大后，可同时容纳多个自由基增长。
\end{itemize}
