...
\begin{enumerate}[leftmargin=2.2em,label=\arabic*.]
  ...
  \item \textbf{制造方式分类}：- 减材制造： DeltaG<0 去除材料（如切削）
...
\begin{itemize}[leftmargin=2.2em,label=--]
  ...
  \item - 等材制造： DeltaG=0 材料质量不变（如铸造）
  \item - 增材制造： DeltaG>0 逐层累加材料（如3D打印）
...
\begin{enumerate}[leftmargin=2.2em,label=\arabic*.]
  ...
  \item \textbf{主运动}：速度最高、消耗功率最大的运动，如车削时工件的
  \item \textbf{进给运动}：将待切金属不断投入切削的运动，如车刀的移动。
  \item \textbf{合成切削运动}：主运动和进给运动的矢量和，决定了切削刃的运动
  \item 计算公式：vc = pidwn / 1000进给量 ( f )
  \item 计算公式：ap = (dw - dm) / 2 (外圆车削)
  \item \textbf{车刀基本组成}：由刀柄（夹持部分）和刀头（切削部分）组成，其中刀头是承担切削任务的核心区域。
  \item \textbf{切削部分结构要素}：前刀面 (Agamma)：切屑流过的刀面后刀面 (Aalpha)：与工件过渡表面相对的刀面副后刀面 (Aalpha')：与工件已加工表面相对的刀面主切削刃 (S)：前刀面与后刀面的交线副切削刃 (S')：前刀面与副后刀面的交线刀尖：主、副切削刃的连接部分 圆弧、线段
  \item \textbf{刀具角度的参考系}：基面Pr ：通过主切削刃上某一指定点，并与该点切削
  \item \textbf{速度方向相垂直的平面。}：切削平面Ps：通过主切削刃上某一指定点，与主切削刃相切并垂直于该点基面的平面。
  \item \textbf{正交平面参考系}：由基面、切削平面和正交平面三个相互垂直的平面组成，
  \item \textbf{工作角度定义}：考虑合成切削运动和实际安装情况后的实际切削角度，区别于刀具的标注角度。
  \item \textbf{进给运动的影响}：进给运动会使刀具的工作前角增大，工作后角减小。
  \item \textbf{刀具安装对工作角度的影响}：当刀尖安装高于或低于工件中心时，会改变基面和
  \item \textbf{具体角度变化规律}：- 刀尖装高：工作前角增大，工作后角减小- 刀尖装低：工作前角减小，工作后角增大
  \item \textbf{公称厚度 (hD)}：垂直于过渡表面的尺寸 hD=fsinkr
  \item \textbf{公称宽度 (bD)}：沿过渡表面度量的切削层尺寸 bD=
  \item \textbf{公称横截面积 (AD)}：切削层在基面上的投影面积 AD=hDbD=fap
\end{enumerate}
\end{tcolorbox}
\noindent\textbf{次重点与补充：}
\begin{itemize}[leftmargin=2.2em,label=--]
  ...
  \item 轨迹。
  \item 切削用量三要素切削速度 (vc)
  \item 切削刃相对于工件的主运动速度，单位m/s或m/min。
  \item 工件或刀具每转一周的相对位移，单位mm/r。
  \item 进给速度：vf=nf=nzfz背吃刀量 (ap)
  \item 刀具切削刃与工件的接触长度在同时垂直于主运动和进给进给运动的方向上的投影值称为背吃刀量，单位mm。
  \item 钻孔 端面铣 圆周铣刀具切削部分的基本定义
  \item 正交平面Po ：通过主切削刃上某一指定点，同时垂直于该点基面和切削平面的平面。
  \item 是最常用的参考系。
  \item 刀具角度详解正交平面内角度
  \item 前角 gammao 前刀面与基面之间的夹角（±）
  \item 后角alphao 前后刀面与切削平面之间的夹角楔角betao 前刀面与后刀面之间的夹角切削平面内角度
  \item 刃倾角lambdas 主切削刃与基面之间的夹角（±）
  \item 决定切屑的流出方向和刀尖的强度基面内角度
  \item 主偏角Kr 主切削刃在基面上的投影与进给方向的夹角副偏角Kr‘ 副切削刃在基面上的投影与进给反方向的夹角刀尖角epsilonr 主切削刃与副切削刃之间的夹角刀具的工作角度
  \item 在车削接近工件中心时，工作后角可能变为负值，导致工件被挤断。
  \item gammaoe=gammao+etaalphaoe=alphao-eta
  \item 切削平面的位置，从而直接影响刀具的工作角度。
  \item 切削层参数切削层定义
  \item 切削刃在一次走刀中从工件上切下的一层材料称为切削层切削层的截面尺寸参数称为切削层参数。
  \item rpa
  \item sin机床的分类和型号编制方法
\end{itemize}
...
\begin{enumerate}[leftmargin=2.2em,label=\arabic*.]
  ...
  \item \textbf{第二变形区（纤维化区）}：切屑沿前刀面排出时，底层金属受强烈摩擦挤压，产生纤维化
  \item \textbf{第三变形区（加工表面变形区）}：工件已加工表面受刃口挤压和后刀面摩擦，产生弹塑性变形，
  \item \textbf{切屑的形成过程及变形区的划分}：OA、OB和OM等都是等剪应力曲线。
  \item \textbf{变形系数Lambdah}：切屑厚度hch与切削层厚度hD之比称为厚度变形系数Lambdaha而切削层长度lc与切屑长度lch之比称为长度变形系数Lambdahl变形系数Lambdah的值是大于1的数。Lambdah越大，变形越大Lambdah与剪切角phi有关，phi增大，Lambdah减小，切削变形减小
  \item \textbf{Deltas}：Deltas NP NK KPepsilon Deltay MK MK
  \item \textbf{剪切角phi}：滑移剪切角phi与切削变形有密切关系，我们也可以用剪切角phi
  \item \textbf{来衡量切削变形的程度}：因为主应力方向与最大剪应力方向的夹角应为45°，即Fs与F的
  \item \textbf{夹角应为45°}：结论：1)前角gammao增大时，剪切角phi随之增大，变形减小。
  \item \textbf{节状切屑}：切削中等塑性材料时形成，切屑上有明显的节状纹路，切
  \item \textbf{崩碎切屑}：切削脆性材料（如铸铁）时形成，切屑呈不规则的碎块状，
  \item \textbf{加工表面粗糙。}：带状切屑 节状切屑 单元切屑 崩碎切屑
  \item \textbf{积屑瘤}：切削钢和铝合金等塑性金属时，在切削速度不高，而又能形成带状切屑的情况下，常在前刀面切削处粘着一块断面呈三角状的硬块，其硬度可达工件材料硬度的2～3倍，能够代替刀面和切削刃进行切削，这个硬块称为积屑瘤。
\end{enumerate}
\end{tcolorbox}
\noindent\textbf{次重点与补充：}
\begin{itemize}[leftmargin=2.2em,label=--]
  ...
  \item AM
  \item O ⅢⅡ Ⅰ
  \item OA称作始滑移线OM称作终滑移线
  \item 一般切削速度，第一变形区的宽度仅为0.02～0.2mm，所以可以用一剪切面来表示。
  \item 剪切面与切削速度方向的夹角称作剪切角，以phi表示A B E G
  \item H′F′
  \item C′D H F C
  \item G′vc
  \item E′B′ 切削层
  ...
  \item Ohch
  \item gammaohD LC
  \item lch 刀具相对滑移epsilon
  \item 相对滑移(剪应变)图中，平行四边形 OHNM发生剪切变形后，变为平行四边形OGPM，其相对滑移K
  \item PN
  \item GH O
  \item gammao phi -gammaoDeltay
  \item ocot tan( )
  \item 2)摩擦角beta增大时，剪切角phi随之减小，变形增大。
  \item betaFm
  \item FnFns
  \item FsFc
  \item Fpbeta-gammao
  \item F切屑类型
  \item 带状切屑切削塑性较好的材料（如低碳钢）时形成，切削过程平稳，
  \item 表面质量好，但不易断屑。
  \item 削过程有轻微振动。
  \item 单元切屑切削塑性较低的材料时形成，切屑呈粒状，切削过程振动
  \item 较大，影响加工精度。
  \item 积屑瘤产生的原因塑性材料
  \item 强烈摩擦较高压力
  \item 温度适当积屑瘤的概念
  ...
  \item Ⅳvc/(m/s)
  \item Hb/mm使刀具前角变大
  \item 使切削厚度变化使加工表面粗糙度增大
  \item 对刀具寿命的影响积屑瘤对切削过程的影响有积极的一面，也有消极的一面。
  \item 精加工时必须防止积屑瘤的产生积屑瘤对切削过程的影响
  \item gammabHb
  \item DeltahDhD
  \item 工件 积屑瘤避开产生积屑瘤的切削速度范围
  \item 降低材料塑性合理使用切削液
  \item 增大刀具前角控制积屑瘤产生的措施
  \item 切削力的来源、合力及其分力
\end{itemize}
...
\begin{enumerate}[leftmargin=2.2em,label=\arabic*.]
  ...
  \item 刀具几何参数的影响- 前角gammao：
  \item \textbf{切削塑性材料时，gammao对切削力的影响较大；}：切削脆性材料时，由于切削变形很小，gammao对切削力的影响不
  \item \textbf{显著。}：-20°-10° 0° 10° 20°gammao
  \item \textbf{主偏角kr对Fc的影响不大，对Fp、Ff的影响较大。}：工艺系统刚性差时，需要利用改变主偏角来减小背向力
  \item \textbf{- 刃倾角lambdas ：}：改变刃倾角将影响切屑在前刀面上的流动方向，从而使切削合
  \item \textbf{力的方向发生变化。}：增大lambdas，Fp减小，Ff增大。lambdas在－45°～10°范围内变化时，
  \item \textbf{0.5}：-45° -40° -30° -20° -10° 0 ° 10 ° lambdas
  \item \textbf{- 刀尖圆弧半径 ：}：刀尖圆弧半径repsilon对Fp、Ff的影响较大，对Fc的影响较小。
  ...
  \item 、 、 、 、 的指数刀具参数：前角gammao对切削温度的影响
  \item 刀具参数：主偏角kr对切削温度的影响减小kr，
  \item \textbf{使切削温度下降。}：30° 45 ° 60 ° 75 ° 90 °kappartheta(℃) vc＝1.75m/s
...
\noindent\textbf{次重点与补充：}
\begin{itemize}[leftmargin=2.2em,label=--]
  \item - 主切削力 Fc ：沿主运动方向，最大分力，消耗功率最多- 进给抗力 Ff ：沿进给运动方向的分力- 背向力 Fp ：沿背吃刀量方向，易使工件弯曲变形刀具
  \item 塑性抗力弹性抗力
  \item 工件切屑
  \item apF
  \item falphafgamma
  \item vc2 2 2 2 2
  \item c p c N fF F F F F F切削力测量与经验公式
  \item 切削力的经验公式（ ap > f > vc ）
  \item -𝐹𝑐=𝐶𝐹𝑐𝐚𝑝
  \item 𝑥𝐹𝑐𝑓𝑦𝐹𝑐𝛖𝑐𝑛𝐹𝑐𝐾𝐹𝑐
  \item 𝐹𝑝=𝐶𝐹𝑝𝐚𝑝𝑥𝐹𝑝
  \item 𝑓𝑦𝐹𝑝𝛖𝑐𝑛𝐹𝑝
  \item 𝐾𝐹𝑝𝐹𝑓=𝐶𝐹𝑓𝐚𝑝
  \item 𝑥𝐹𝑓𝑓
  ...
  \item ap和 f 增大，都会使切削力增大，但两者的影响程度不同。
  \item ap增大时，变形系数Lambdah不变，切削力成正比增大；
  \item f 增大时， Lambdah有所下降，故切削力不成正比增大。
  \item 2.21.8
  \item 1.41.0
  \item Fc(KN)
  \item 切削铸铁等脆性材料时，被切材料的塑性变形及它与前刀面的摩擦均比较小，vc对切削力没有显著影响
  \item 0 20 40 60 80 100 120 1401.6
  \item 1.2Fc(kN)
  \item gammao增大，Lambdah减小，切削力下降。
  \item 3.22.8
  \item 2.42.0
  \item 0.80.4
  \item F(kN)
  \item FcFf
  \item Fp- 负倒棱 ：
  \item bgamma1＜lf 时，切削力虽有所增加，但增大幅度不大；
  \item bgamma1＞lf 时，相当于用负前角为gammao1的车刀进行切削，切削力显著增加。
  \item (a) 刀－屑接触长度lf (b) bgamma1＜lf (c) bgamma1＞lfvc vc vc
  \item gammao gammao1 gammao gammao1 gammaolf bgamma1 bgamma1 vch vch vch- 主偏角kr ：
  \item 主偏角kr增大，背向力Fp减小，进给力Ff增大。
  \item Fc基本不变。
  \item 2.51.5
  \item 当repsilon增大时，平均主偏角减小，故Fp减小，Ff增大。
  ...
  \item az y x v f a
  \item 刀具与切削接触区平均温度（ C）
  \item 切削温度系数切削速度 m/min
  \item 进给量 mm/r背吃刀量 mm
  \item gammao增大，变形减小，
  \item 切削力减小，切削温度下降。
  \item -10° 0 ° 10 ° 20 ° 30 °gammao2 3
  \item 切削刃工作长度和刀尖角增大，散热条件变好，
...
\begin{enumerate}[leftmargin=2.2em,label=\arabic*.]
  ...
  \item \textbf{𝑣60𝑗}：𝐾𝑟 ——某种材料相对加工性能指标；
...
\begin{itemize}[leftmargin=2.2em,label=--]
  ...
  \item 8 很难切削的材料 <0.15 铸造镍基高温合金、某些钛合金𝐾𝑟= 𝑣60
  \item 𝑣60 ——某种材料其刀具寿命为60min时的切削速度；
  \item 𝑣60𝑗 ——切削45钢，刀具寿命为60min时的切削速度。
...
\begin{enumerate}[leftmargin=2.2em,label=\arabic*.]
  ...
  \item \textbf{常用涂层材料}：TiN (氮化钛)：金黄色，应用广泛，适用于加工低合金钢和不锈钢TiCN (碳氮化钛)：硬度更高，灰色，适用于各种工件材料。
...
\begin{itemize}[leftmargin=2.2em,label=--]
  ...
  \item 高速钢 - 涂层高速钢先进工艺
  \item 采用PVD（物理气相沉积）技术，在刀具表面涂覆一层厚度为2\textasciitilde{}5mum的TiN、TiC、TiCN等硬膜性能优势
...
\begin{tcolorbox}[colback=yellow!6,colframe=red!55!black,title=重点,left=1mm,right=1mm]
\begin{enumerate}[leftmargin=2.2em,label=\arabic*.]
  \item \textbf{液滴直径小于0.1mum}：- 兼具润滑性与冷却清洗性- 无异味、无毒、稳定性好
...
\begin{enumerate}[leftmargin=2.2em,label=\arabic*.]
  ...
  \item \textbf{加工精度}：- 精车外圆精度：0.01mm- 圆柱度：0.01mm/100mm- 表面粗糙度Ra：1.25\textasciitilde{}2.5mum
...
\begin{enumerate}[leftmargin=2.2em,label=\arabic*.]
  ...
  \item \textbf{生产率高，一般用于大批量生产。}：加工质量和精度稳定 加工零件精度可达IT8～IT7，表面粗糙度 Ra 可达 10～
  \item \textbf{2.5mum。}：刀具使用寿命长 成形车刀磨损后，一般只
...
\begin{enumerate}[leftmargin=2.2em,label=\arabic*.]
  ...
  \item \textbf{加工精度较高}：精铣表面粗糙度Ra可达3.2\textasciitilde{}1.6mum，尺寸精度可
...
\begin{enumerate}[leftmargin=2.2em,label=\arabic*.]
  ...
  \item 麻花钻是最常用的孔加工刀具，一般用于实体材料上孔的粗加工钻孔的尺寸精度为 IT11～IT13，表面粗糙度 Ra 为 50～12.5mum麻花钻的切削部分和导向部分：
  \item \textbf{钻孔刀具 – 扩孔钻}：扩孔钻是用来对工件上已有孔进行扩大加工的刀具。
  \item 扩孔后，孔的精度可达到 IT9\textasciitilde{}IT10,表面粗糙度 Ra可达到6.3\textasciitilde{}3.2mum结构特点：
  \item \textbf{钻孔刀具 – 铰刀}：铰刀是一种半精加工或精加工孔的常用刀具铰孔后孔的精度可达IT7 \textasciitilde{}IT9，表面粗糙度 Ra可达1.6\textasciitilde{}0.4mum
...
\begin{itemize}[leftmargin=2.2em,label=--]
  ...
  \item 扩孔后，孔的精度可达到 IT9\textasciitilde{}IT10,表面粗糙度 Ra可达到6.3\textasciitilde{}3.2mum 扩孔除了可以加工圆柱孔之外，还可以用各种特殊形状的扩孔钻（亦称锪钻）来加工各种沉头座孔和锪平端面
...
\begin{tcolorbox}[colback=yellow!6,colframe=red!55!black,title=重点,left=1mm,right=1mm]
\begin{enumerate}[leftmargin=2.2em,label=\arabic*.]
  \item \textbf{加工特点：}：镗削主要用于工件上已有铸造孔或加工孔的后续加工，常用于加工尺寸较大及精度要求较高的场合，特别适宜加工尺寸精度和位置精度要求十分严格的孔系镗削是常用的加工方法，其加工范围很广，既可进行粗加工，也可进行精加工。镗削加工的标准公差等级一般为IT6\textasciitilde{}IT7, 表面粗糙度 Ra一般为6.3\textasciitilde{}0.8mum
  \item 镗刀 – 单刃镗刀结构特点：
...
\noindent\textbf{次重点与补充：}
\begin{itemize}[leftmargin=2.2em,label=--]
  \item 轴向进给运动 (fa) - 相对移动工件相对于砂轮的轴向移动，决定磨削的宽度范围工件运动 (vw) - 旋转/移动工件自身的旋转或直线移动，提供进给速度以配合切削𝑣= 𝜋𝑑0𝑛0
  \item 磨削力与磨削功率
\end{itemize}
...
\begin{enumerate}[leftmargin=2.2em,label=\arabic*.]
  ...
  \item \textbf{消}：安装误差：本工序定位基准与装夹带来的偏差𝛥𝑍总=𝛥𝑍1+𝛥𝑍2+𝛥𝑍3+⋯+𝛥𝑍𝑛=
...
\noindent\textbf{次重点与补充：}
\begin{itemize}[leftmargin=2.2em,label=--]
  \item 加工余量的确定加工总余量 (DeltaZ总)
  \item 毛坯与成品设计尺寸之差，即切除金属层总厚度工序余量 (DeltaZi)
  \item 相邻两工序尺寸之差，即单工序切除厚度。
  \item - 单边余量：平面加工- 双边余量：轴/孔、回转面
  \item 𝑛𝑖<1
  \item 𝛥𝑍𝑖机械制造技术基础C 第八章 机械加工工艺规程的制订
...
\begin{enumerate}[leftmargin=2.2em,label=\arabic*.]
  ...
  \item \textbf{𝐿1=40:0.05}：:0.09𝑚𝑚= 40.07±0.02𝑚𝑚
  \item \textbf{𝐸𝐼1=𝐸𝑆2−𝐸𝑆0}：𝐸𝐼1=0−+0.25 𝑚𝑚=−0.25𝑚𝑚
  \item \textbf{;0.15𝑚𝑚}：按入体原则标注，则为 𝐴1=39.85;0.1
  \item \textbf{0 𝑚𝑚。}：尺寸链的计算 – 测量基准与设计基准不重合时
  \item \textbf{其中 40;0.3}：0 𝑚𝑚为间接得到的，为封闭环𝐿0
  \item \textbf{𝐿1为减环。}：𝐿1= 40−10𝑚𝑚=30𝑚𝑚𝑇1= 0.3−0.16𝑚𝑚=0.14𝑚𝑚
  \item \textbf{=9.8𝑚𝑚}：𝐸𝑆0=𝐸𝑆1+𝐸𝑆3−𝐸𝐼2𝐸𝑆1=𝐸𝑆0+𝐸𝐼2−𝐸𝑆3=0.18+−0.09−0𝑚𝑚
  \item \textbf{=0.09𝑚𝑚}：𝐸𝐼0=𝐸𝐼1+𝐸𝐼3−𝐸𝑆2𝐸𝐼1=𝐸𝐼0+𝐸𝑆2−𝐸𝐼3=−0.18+0−−0.09 𝑚𝑚
  \item \textbf{=−0.09𝑚𝑚}：所以 𝐴1=9.8𝑚𝑚±0.09𝑚𝑚
\end{enumerate}
\end{tcolorbox}
\noindent\textbf{次重点与补充：}
\begin{itemize}[leftmargin=2.2em,label=--]
  ...
  \item 尺寸链的计算 - 定位基准与设计基准不重合时尺寸 𝐿1是本工序保证的，尺寸 100
  \item :0.18𝑚𝑚 要通过前面三个尺寸间接保证，所以 𝐿0=100
  \item :0.18𝑚𝑚是封闭环。
  \item 𝐿1、𝐿2为增环， 𝐿3为减环𝐿0=𝐿1+𝐿2−𝐿3
  \item 10𝑚𝑚=𝐿1+30𝑚𝑚−60𝑚𝑚𝐿1=40𝑚𝑚
  \item 𝐸𝑆0=𝐸𝑆1+𝐸𝑆2−𝐸𝐼30.18𝑚𝑚=𝐸𝑆1+0.04𝑚𝑚−−0.05𝑚𝑚𝐸𝑆1=+0.09𝑚𝑚
  \item 𝐸𝐼0=𝐸𝐼1+𝐸𝐼2−𝐸𝑆30𝑚𝑚=𝐸𝐼1+0𝑚𝑚−+0.05𝑚𝑚𝐸𝐼1=0.05𝑚𝑚
  \item 尺寸链的计算 – 定位基准与设计基准不重合时𝐴0400
  \item :0.25𝑚𝑚 为封闭环，𝐴1为减环，
  \item 𝐴280;0.150 𝑚𝑚 为增环
  \item 𝐴0=𝐴2−𝐴140𝑚𝑚=80𝑚𝑚−𝐴1
  \item 𝐴1=40𝑚𝑚𝐸𝑆0=𝐸𝑆2−𝐸𝐼1
  \item 𝐸𝐼0=𝐸𝐼2−𝐸𝑆1𝐸𝑆1=𝐸𝐼2−𝐸𝐼0
  \item 𝐸𝑆1= −0.15−0𝑚𝑚=−0.15𝑚𝑚𝐴1=40;0.25
  \item 而 40;0.160 𝑚𝑚为增环，
  \item 𝐸𝑆1=0.14𝑚𝑚𝐸𝐼1=0
  \item 所以，尺寸 𝐿1=300:0.14𝑚𝑚。
  \item 尺寸链的计算 – 需继续加工的表面标注的工序尺寸计算画尺寸链
  \item 𝐴0是间接形成的封闭环，𝐴1、𝐴3为增环，
  \item 𝐴2为减环。
  \item 公称尺寸：
  \item 𝐴0=𝐴1+𝐴3−𝐴2𝐴1=43.4𝑚𝑚
  \item 上极限偏差：
  \item 𝐸𝑆0=𝐸𝑆1+𝐸𝑆3−𝐸𝐼2𝐸𝑆1=+0.315𝑚𝑚
  \item 下极限偏差：
  \item 𝐸𝐼0=𝐸𝐼1+𝐸𝐼3−𝐸𝑆2𝐸𝐼1=+0.05𝑚𝑚
  \item 所以 𝐴1=43.4:0.05:0.315𝑚𝑚
  \item 入体尺寸原则写为 𝐴1=43.450:0.265𝑚𝑚。
  \item 尺寸链的计算 – 对多尺寸保证的工艺尺寸链计算3工序钻 4工序车 5工序磨𝐴010𝑚𝑚±0.18𝑚𝑚 为封闭环，A1、A3为增环, A2为减环。
  \item 𝐴1=𝐴0−𝐴3+𝐴2= 10−9.2+9𝑚𝑚
...
\begin{tcolorbox}[colback=yellow!6,colframe=red!55!black,title=重点,left=1mm,right=1mm]
\begin{enumerate}[leftmargin=2.2em,label=\arabic*.]
  \item \textbf{基准不重合误差 (DeltaB)}：由于工序基准和定位基准不重合而引起的误差DeltaB的大小为从定位基准到设计基准之间在加工尺寸方向上的最大变化量，在工序尺寸方向上
  \item 的投影基准位移误差 (DeltaY)
  \item 由于定位元件或工件制造不准确，导致定位基准在工序尺寸方向上发生位置偏移而产生的误差。
  \item \textbf{定位误差公式：}：DeltaD = DeltaB + DeltaY (矢量和)
\end{enumerate}
\end{tcolorbox}
\noindent\textbf{次重点与补充：}
\begin{itemize}[leftmargin=2.2em,label=--]
...
\begin{itemize}[leftmargin=2.2em,label=--]
  ...
  \item 导轨在垂直平面内有直线度误差Deltaz时，也会使车刀在垂直面内发生位移，使工件半径产生误差DeltaR, DeltaR\textasciitilde{}（Deltaz)2/(2R)。
  \item 与Deltaz值相比，DeltaR属微小量，一般可忽略不计2. 水平面内的直线度（核心）
  \item 导轨在水平面内有直线度误差Deltay时，工件半径产生DeltaR=Deltay的误差。处于误差敏感方向，影响极大，直接决定加工尺寸精度。
  \item 时，刀尖的运动轨迹是一条空间曲线，使工件产生圆柱度误差。
  \item 导轨间在垂直方向有平行度误差Deltal3时，将使工件与刀具的正确位置在误差敏感方向产生Deltay\textasciitilde{}（H/B) ×Deltal3的偏移量，使工件半径产 生DeltaR＝ Deltay的误差，对加工精度影响较大。
...
\begin{enumerate}[leftmargin=2.2em,label=\arabic*.]
  ...
  \item 公式：W = 1 / K (mm/N)
  \item \textbf{𝐾𝑔}：Kst、 Kjc、 Kj、 Ka、 Kg——工艺系统、机床、夹具、
  \item \textbf{误差复映}：待加工表面上有什么样的误差，加工表面上必然也有同样性质的误差，这就是切削加工中的误差复映现象
  \item \textbf{误差复映系数}：epsilon称为误差复映系数，其值为小于1的正数。
...
\begin{itemize}[leftmargin=2.2em,label=--]
  ...
  \item 数值越小，加工精度越高。
  \item 工艺系统总变形系统总变形 =
  \item 机床变形 + 夹具变形 + 刀具变形 + 工件变形𝑌𝑠𝑡=𝑌𝑗𝑐+𝑌𝑗+𝑌𝑑+𝑌𝑔𝑌𝑠𝑡—工艺系统的变形量；
  \item 𝑌𝑗𝑐—机床的变形量；
  \item 𝑌𝑗—夹具的变形量；
  \item 𝑌𝑑—刀具的变形量；
  \item 𝑌𝑔—工件的变形量𝐾𝑠𝑡= 1
